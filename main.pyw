...
class PygameWindow():
    ...
    def update(self):
        ...
        while self.is_running:
            ...
            if self.GAME_START==True:
                ...
                if inf[9]!=self.bullet.angle:   self.bullet.UpdateRotation(inf[9])
                if inf[10]!= self.opponentbullet.angle:     self.opponentbullet.UpdateRotation(inf[10])
    ...
    def SpawnPlayers(self,opponentname,playerType=1):
        ...
        self.tank=Tank(self.screen,self.imageDict['tank1'],False,0.1,0.6,True)
...
class Sprite():
    def __init__(self,screen,image=None,isCollider=False,pivotX=0,pivotY=0,debugBBox=False):
        ...
        if self.image!=None and isCollider==False:
            self.collider=BoundingBox(self.screen,self.imagewidth,self.imageheight,self.pX,self.pY,debugBBox)
